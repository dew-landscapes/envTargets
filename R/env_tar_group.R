...
#' into account one of the columns (`column`) within the dataframe (usually `n`)
#' that is likely to be correlated with processing time. Remember to set
#' `iteration = "group"` within the target.
#'
#' @param df dataframe
#' @param column column to use when assigning rows to groups
...
env_tar_group <- function(df
  ...
  arr <- df |>
    dplyr::arrange(desc(!!rlang::ensym(column)))
